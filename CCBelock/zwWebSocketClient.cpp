...
namespace {
	class WebSocketRequestHandler:public Poco::Net::HTTPRequestHandler {
	      ...
	      WebSocketRequestHandler(std::size_t bufSize = 1024):_bufSize(bufSize)
		{
			...
			try {
				...
				std::auto_ptr <
				    char >pBuffer(new char[_bufSize]);
				int flags;
				int n;
				do {
					n = ws.receiveFrame(pBuffer.get(),
							    _bufSize, flags);
				...
				case WebSocket::WS_ERR_HANDSHAKE_NO_KEY:
					response.
					    setStatusAndReason(HTTPResponse::
							       HTTP_BAD_REQUEST);
	...
	class WebSocketRequestHandlerFactory:public Poco::Net::
...
{
	assert(host != NULL && strlen(host) > 0 && port > 0
	       && port < (256 * 256));
	cs.setHost(host);
...
void zwWebSocket::wsConnect(void)
{
	try {
		...
		OutputDebugStringA
		    ("JC WebSocket Connect Failed 20140813.1955!");
		throw;
	}
...
int zwWebSocket::SendString(const string & str)
{
	...
	int flags =
	    ws->sendFrame(str.data(), str.length(), WebSocket::FRAME_TEXT);
	return flags;
}
